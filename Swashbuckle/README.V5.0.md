...
为WebApi项目提供 [Swagger](http://swagger.io/) 无缝支持! Combines ApiExplorer and Swagger/swagger-ui to provide a rich discovery, documentation and playground experience to your API consumers.
...
* 自动生成 [Swagger 2.0](https://github.com/swagger-api/swagger-spec/blob/master/versions/2.0.md) 文档
* 无缝集成swagger-ui
* 基于反射架构API类型描述生成
...
* 支持描述ApiKey, 基础Auth以及OAuth2架构 ... 包括显式OAuth2流程的UI支持。
...
如果您的服务托管在 OWIN 中间件之上，只需安装 Core 组件库。


    Install-Package Swashbuckle.Core

然后在一个上HttpConfiguration的实例上（例如，在Startup.cs）通过调用扩展方法（在命名空间Swashbuckle.Application中）来手工启用Swagger文档以及swagger-ui。
...
在本示例中swagger-ui的地址为`sandbox/index`.
...
### Swagger-ui 在 Visual Studio 2013 中失效 ###
...
### OWIN托管于IIS - Incorrect VirtualPathRoot Handling
...
Swagger 2.0 允许在文档的任意部位添加额外的元数据信息 (aka 供应商扩展) . Swashbuckle supports this by including a "vendorExtensions" dictionary with each of the extensible Swagger types. Meta-data can be added to these dictionaries from custom Schema, Operation or Document filters. 例如:
